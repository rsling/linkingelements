...
While it became clear during our analysis that the automatic analyser makes some errors and is not very good at disambiguating at least some types of ambiguous compounds, it is clear that the quality is more than sufficient for the kind of large-scale pre-analaysis we performed.
...
In Table~\ref{tab:typeoverview}, we show the type frequencies of first elements (N1) as well as compounds formed with those N1s, grouped by the different linking elements.
