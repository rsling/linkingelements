...
This information tells us how strongly first constituents tend to appear pluralic linking elements and with non-pluralic links.
...
For each of the pluralic linking elements (PL), we generated exhaustive lists of the N1s occurring with it which fulfil the criteria mentioned in Section~\ref{sec:linkingelementsingerman} (prominently: no mass nouns, no weak masculine nouns).%
...
For N1s with \textit{-e} and \textit{=w}, and even more so for those with \textit{=er}, \textit{-er}, and \textit{=}, the overall pattern seems to be that the productivity scores with PL are spread out between $0$ and $1$.
