...
\footnote{It is also available free of charge to anyone working in academia.
...
The only other available very large corpus containing recent German would be the \textit{Deutsches Refernzkorpus} (DeReKo) by the \textit{Institut für Deutsche Sprache} \parencite{KupietzEa2010}, but (at least currently) it is not available for scripted access, and it mostly contains newspaper text.
...
For the studies reported in Section~\ref{sec:corpusstudy} and~\ref{sec:split100experiment}, we had to choose a set of first constituents (N1s) to be examined more thoroughly using corpus data and a set of compounds to be used as the items in the split-100 experiment.
...
For each of the resulting seventy N1s, the design of the study consisted in drawing samples of one hundred occurrences with that pluralic linking element and one hundred occurrences without that linking element.
This was to ensure that all linking elements were represented appropriately in the sample, and it limited the maximal expected size of the concordance for manual annotation to 14,000 lines, which is large but manageable.%
\footnote{In Section~\ref{sec:queries}, we explain why the final sample ended up being slightly smaller.}
For the experiment to be reported in Section~\ref{sec:split100experiment}, an even considerably smaller selection of twelve N1s was required (see Section~\ref{sec:designchoiceofstimuliandparticipants} for the details on why that number turned out to be twelve).

However, choosing those relatively few items from the thousands of possible nouns functioning as N1 is not trivial.
Most importantly, we had to find nouns which are used productively as N1 both with the pluralic linking element and without it, \ie which have the capability of forming new compounds with PL and NPL.
Some N1s (like \textit{Kind} `child' or \textit{Huhn} `chicken') do not alternate between those two alternatives and always occur with the linking element, either by way of lexical preference or because of phonotactic constraints.%
\footnote{It should be noted that many theories (prominently Collostructional Analysis; \citealt{StefanowitschGries2003,Gries2012a,Gries2015b}) predict and have successfully demonstrated that lexical preferences are nuanced and need to be quantified numerically.
Thus, resorting to categorical intuitive judgements as to which N1s are used productively with and without a pluralic linking element are out of the question.}
...
While it would be unlikely that enough N1s exist which have (almost) exactly the same productivity with NP and NPL, any sample should at least be representative of the whole spectrum of productivity.
To obtain such a sample later, we used methods of automatic data extraction with manual quality control to analyse literally \textit{all} compounds in the DECOW16A corpus.
The resulting database informs us how strongly first constituents tend to appear with pluralic linking elements (PLs) and with non-pluralic linking elements (NPLs), and how productively they are used with these linking elements.
...
%
%\footnote{We made an exception for the zero linking element because the number of N1s was much too high for %full manual inspection.
...
%These criteria led to $\Sexpr{nice.int(nrow(blacklists$nul))}$ of $\Sexpr{nice.int(freq.ov["0","N1s"]+nrow(blacklists$nul))}$ nouns being excluded from the list of zero linking element N1 candidates.}
...
These lists of N1 candidates were extracted automatically, but they were checked manually to remove erroneous matches common with methods of automatic extraction.
...
Then, for each of these N1 we counted the numbers of compound types, the number of compound tokens, and the number of compound hapax legomena (\ie those compounds containing the N1 which occur only once in the corpus) containing it.
...
This table is important inasmuch as the numbers reported in it are in line with previous reports based on much smaller corpora such as \textcite{WellmannEa1974}, \textcite{Kuerschner2005}, or \textcite{KrottEa2007} (see also \citealt[9]{Schluecker2012}).
...
As pointed out at the beginning of this section, an informed choice of items for the corpus study and the experiment required ensuring that the chosen N1s had the capability of forming new compounds with PL and NPL.
...
As shown in (\ref{eq:prod}), the potential productivity $P^p$ is simply the number of the hapax legomena ($f_1$) of compounds with $\mathrm{N1}_{\lambda}$ divided by the token frequency ($f$) of compounds with $\mathrm{N1}_{\lambda}$, where $\lambda$ is a (meta)variable standing for either N1's PL or its NPL.

\begin{equation}\label{eq:prod}
P^p(\mathrm{N1}_{\lambda})=\frac{f_1(\mathrm{N1}_{\lambda})}{f(\mathrm{N1}_{\lambda})}
\end{equation}

The interpretation of $P^p$ is intuitive, as it is $0$ when there are no hapax legomena in the corpus ($f_1(\mathrm{N1}_{\lambda})=0$; no productivity whatsoever), and it is $1$ if all tokens are hapax legomena ($f_1(\mathrm{N1}_{\lambda})=f(\mathrm{N1}_{\lambda})$; maximal productivity).
...
Figure~\ref{fig:proddots} shows the results of the productivity analyses for all  \Sexpr{nice.int(sum(unlist(le.aggregated[1,])))} chosen N1s.
%\footnote{It was pointed out that the analysis of productivity is often tainted by the low quality of automatic annotation in \textcite{EvertLuedeling2001} -- a point reiterated in \textcite[907]{Baayen2009}.
...
%Also, we have found that the compound analyses by the SMOR tool are highly accurate when it does not have to guess the lemma of a compound's elements, and the N1s actually used are obviously known words, since we input them.
...
In the extreme case, a N1 has a type frequency of $1$ as an N1, and there is also only one occurrence of it (necessarily a hapax legomenon) as N1, which results in a potential productivity score of $1$.
Since the \textit{-n} and \textit{-en} plurals (of N1s) are often used with rare loan words (as N1), there are many items with low type frequency and a high productivity score.
