...
% \end{center}
% \caption{Comparisons of the potential productivity ($P^p$) of first constituents, grouped by their PL; colors and sizes encode the type frequencies with PL and NPL; axes are on a logarithmic scale}
% \label{fig:proddots}
% \end{figure}
