...
\footnote{See \url{https://dx.doi.org/10.5281/zenodo.1322791} for a copy of the full data set and all scripts used in the research presented here.}
...
For the experiment to be reported in Section~\ref{sec:split100experiment}, an even considerably smaller selection of twelve N1s was required (see Section~\ref{sec:designchoiceofstimuliandparticipants} for the details on why that number was twelve).
...
To obtain such a sample later, we first used methods of automatic data extraction (with manual quality control) to analyse literally \textit{all} compounds in the DECOW16A corpus.
The resulting database informs us about how strongly first constituents tend to appear with pluralic linking elements (PLs) and with non-pluralic linking elements (NPLs), and how productively they are used with these linking elements.
...
%We therefore implemented an automatic approach where an N1 had to fulfil one of the following criteria to be included:
...
Then, for each of these N1 we counted its number of compound types, its number of compound tokens, and its number of compound hapax legomena (\ie those compounds containing it which occur only once in the corpus).
