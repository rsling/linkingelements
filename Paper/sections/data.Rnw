...
Since we are interested in the alternation for each N1 of PLs and non-pluralic linkages (NPLS; these can manifest themselves as no LE at all, as deletion, or as a non-pluralic LE such as \textit{-s}), we also extracted the same counts for the N1s in compounds with a NPL.
