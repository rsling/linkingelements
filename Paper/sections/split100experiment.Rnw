...
The experiment was designed to examine how strongly native speakers of German prefer a pluralic link in two sub-experiments (internal and external plural).
...
In a split-100 task, subjects are offered two options (in the case at hand: a compound with PL or NPL) and they can weigh their preference for either of them, assigning integer values between $100;0$ (clear preference for option one) and $0;100$ (clear preference for option two) to the tuple of options.
...
There was a training phase with five sentences\slash decisions.
...
The process turned out to be an iterative one because compounds meeting the desired criteria were difficult to find.
Table~\ref{tab:stimuli} presents the results of the selection process.
...
The frequency classes $c$ of the fuli compound with PL and NPL is also given, where class $28$ corresponds to a token frequency of $1$, and no frequency class (--) is assigned to words with a token frequency of $0$.
...
The top eight compounds shown in the table were used as stimuli for the internal plural sub-experiment.
...
On the other hand, \textit{Eingang} `entry', \textit{Beschriftung} `label(ling)', \textit{Schliff} `cut\slash sharpening', as well as \textit{Abdruck} `mark\slash impression (physical)' cannot trigger internal plural semantics at all.
Because it is sensitive to the specific semantic relation between N1 and N2, the internal plural sub-experiment might be sensitive item-specific effects pertaining both to concrete combinations of N1 and N2.
...
Since both combinations were used with PL and NPL, this would have meant exposing subjects to sixteen targets for the internal plural sub-experiment alone.
Therefore, randomisation was used, and participants saw either the version with PL or NPL.
For the external plural sub-experiment, we only used N2s which could not trigger internal plural, and we therefore decided that increased variety of N1s was better, using eight different N1s.
...
The external plural sub-experiment (right panel of Figure~\ref{fig:conditions}) obviously had a negative result.
...
Worse, however, subjects tend to assign ratings of $0$ and $100$ very often (see Figure~\ref{fig:participants}), which leads to so-called \textit{zero inflation} and \textit{one inflation} (see \citealt{ZuurEa2009} for a detailed account of practical modelling with zero inflation targeted at practitioners).
...
It reaches $sig=0.05$ ($t=\Sexpr{nice.float(model.coll.sum$coef.table["ConditionIndividual","t value"])}$).
