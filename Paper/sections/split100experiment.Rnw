...
Additionally, the total difference in token frequency of the two compounds is specified ($\Delta_f$), where the maximum absolute difference is $63$, which is clearly not exrteme, given that the corpus contains 21 billion tokens and approximately 15 billion words.
...
No money was paid, but participants received credit in partial fulfilment of the requirements for the abovementioned classes.
...
The external plural did not trigger PL more than NPL, but there is solid evidence that speakers favour a PL in compounds where an internal plural relation necessarily holds betweenN1 and N2.
