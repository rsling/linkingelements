...
The experiment was designed to find out how strongly native speakers of German prefer a pluralic link in one of the two conditions (internal or external plural).
...
First of all, we chose N1s which have a similar productivity with PL and NPL.
...
%<<experimentselection, fig.pos="htb!", fig.height=8, fig.cap="Selection of N1 for the stimuli in the split-100 experiment in the context of all possible candidates", fig.show='hide', message=FALSE, warning=FALSE, results='hide', error=FALSE>>=
...
colnames(stims.freqs)[c(11,12)] <- c("Prod-PL", "Prod-NPL")
...
stims.tab <- stims.freqs[, c("N1", "LE", "N2", "Prod-PL", "Prod-NPL", "FbandPL", "FbandNPL", "AbsFDiff", "Gloss")]
stims.tab[, c("Prod-PL")] <- nice.float(stims.tab[, c("Prod-PL")])
stims.tab[, c("Prod-NPL")] <- nice.float(stims.tab[, c("Prod-NPL")])
colnames(stims.tab)[c(6,7,8)] <- c("c-PL", "c-NPL", "Delta-f")
stims.xt <- xtable(stims.tab,
             caption = "Selection of stimuli for split-100 experiment; Prod-PL and Prod-NPL are the potential productivities of the N1 with PL and NPL, respectively; c-PL and c-NPL are the frequency bands of the compound with PL and NPL, respectively; Delta-f is the difference in raw token frquency f(PL)-f(NPL)",
             label = "tab:stimuli")
