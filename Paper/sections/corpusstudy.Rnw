...
The most widely used statistical system is \textit{Null Hypothesis Significance Testing} (NHST).
It attempts to substantiate the existence of effects (such as a positive connection between plurality and pluralic linking elements) by conducting experiments in which the effect is measured in some way.
...
If this probability is lower than a certain threshold (usually called the \textit{alpha level}), the null hypothesis is \textit{rejected}, which is taken as evidence that there is a true effect.
...
vioplot2(t.plot$phi.strict,
         ...
         col = "orange",
         add = T)
title("Signed effect strengths\nin number and plural induction conditions",
      xlab = "Condition")
