...
As explained in Section~\ref{sec:linkingelementsingerman}, we will determine whether two distinct factors increase the probability that N1 in an N1+N2 compound occurs with a PL, given that N1 alternates between a PL and a NPL.
...
Ideally, we would have had ten per PL, but since some of the PLs occurred with relatively few N1s, there were not always ten nouns available which met our conditions.
In those cases, we took as many as we could.
The sampling process consisted of sorting manually through all nouns which occurred with a given PL and removing those which did not meet our requirements.
At the end, we were left with a set of fifty nouns that were countable (\ie no mass nouns), not collectives, and distributed approximately uniformly across the productivity spectrum as it was visualised in Figure~\ref{fig:proddots}.
...
Additionally, we performed deduplication on the compounds inasmuch as we only allowed one instance of each identical compound word form to pass.
...
In addition to these fairly clear-cut cases, there was a second group of compounds in which the plural-inducing quality of the N2 was strongly dependent on context and world knowledge.
...
However, since the interpretation of this kind of containment compound is very context- and world-knowledge-dependent, we refer to these as extended cases and will analyse them separately from the clear-cut cases shown above.
...
We feel that corpus linguists have not yet picked up on the debate fully, with the effect that most papers do not make explicit which statistical philosophy they follow.
...
The question is whether there is an considerably high number of exemplars with PL in a plural compound such that we might conclude that there is an external plural effect.
...
\footnote{Violin plots are extensions of standard box plots; see \textcite{HintzeNelson1998}.
