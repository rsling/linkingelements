...
In NHST, researchers attempts to substantiate the existence of effects (such as a positive connection between plurality and pluralic linking elements), which is predicted to exist by some theory, by conducting experiments in which the effect is measured.
Then, it is calculated how likely it was \textit{before the experiment was conducted} to obtain the actual measurements if there is actually \textit{no} effect (the \textit{null hypothesis} or just the \textit{null}).
If this probability is lower than a certain threshold (usually called the $\alpha$-\textit{level}), the null hypothesis is \textit{rejected}, which is taken as evidence that there is a true effect.
This approach is riddled with conceptual problems and has led to the promotion of of bad scientific practice.
Among the most ardent critics are \textcite{Gigerenzer2004}, \textcite{Colquhoun2014}, and \textcite{MunafoEa2017}, and the editors of the journal \textit{Basic and Applied Social Psychology} have even banned the use of p-values in an actionist attempt to tackle problems of bad science due to NHST \parencite{TrafimowMarks2016}.
...
However, there is no need to abandon frequentism of p-values simply because they have been abused.
...
If this is possible, then researchers can calculate the risk of incorrectly accepting H\Sub{M} when it is actually false ($\alpha$) and the risk of incorrectly accepting H\Sub{A} when it is actually false ($\beta$) \textit{given a specific sample size}.
...
While Fisher did not directly recommend the inspection or p-values, he recommended that experimenters set \textit{sig} appropriately based on previous experimental or theoretical knowledge (see Chapter~4 of \citealt{Lehmann2011} and \citealt{Perezgonzalez2015}).
Furthermore, p-values can be corrected after the experiment, for examples if many tests are performed, which increases the actual error rates relative to the nominal ones.
...
\begin{enumerate}
  ...
  \item \label{it:fisher04} If one does a series of experiments and corresponding tests where a connected set of nulls is evaluated, the actual probabilities of just a rare event happening increase, and each $p$ or \textit{sig} is too optimistic.
...
Points~(\ref{it:fisher01}) and~(\ref{it:fisher02}) can be remedied by researchers being aware of the actual (low) importance which a single significant results has.
...
Problem with point~(\ref{it:fisher03}) are easily avoided by looking at effect sizes (usually associated with the Neyman-Pearson approach, but imported easily into a Fisherian procedure), which is really just another way of saying that researchers should to proper exploratory analysis of their data sets.
...
Finally, point~(\ref{it:fisher05}) poses the most serious threat to the validity in corpus linguistics.
Fisher's logic of experimental design assumes that test subjects were randomly chosen from the population of interest and that thus confounding factors are distributed randomly (see Chapter~2 of \citealt{MaxwellDelaney2004} for a convenient overview).
Experimentation in corpus linguistics but also in linguistics in general is marred by the fact that researchers usually cannot even specify their population of interest.
The traditional discussion of the \textit{representativeness} of a corpus does not help because it is more often than not centered around the concept of a corpus being ``representative of a language'', using as points of reference (i) the distribution of texts or text types in the output of all speakers of a language (production-based), (ii) the distribution of the relevance of texts or text types in the whole speech community (relevance-based), or (iii) the distribution of the exposition events of speakers to different texts or text types (perception-based).%
...
The population of interest has to be defined with regard to each specific experiment, and it might be something very specific (such as the written output of speakers of a certain age, in a specific register, etc.) instead of ``the language'' or ``the average speaker'' (across all communicative settings and modes).
In social sciences, the labels of \textit{global and specific representativeness} \citep[86]{Bortz2005} are used to describe the relevant distinction.
...
Such an approach is not wrong per se, and we include our own work in the same category.
...
(i) choose the most varied and large corpus available (where variedness outweighs size),
(ii) consider their work mostly exploratory, even if statistical tests are used and previous theoretical knowledge and predictions are used,
(iii) be appropriately careful with their interpretation.
