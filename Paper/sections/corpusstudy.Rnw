...
%Concretely, we required that the potential productivity score with PL and with NPL not be $0$ or $1$, which excluded rare words.
Furthermore, it was explained in Section~\ref{sec:databaseandproductivityassessment} that the productivity measures can be misleading in the case of low type frequencies.
...
The queries we used to retrieve the exemplars of compounds containing the 48 chosen N1s were made in DECOW16A.
%using Python through the RStudio Server interface provided by the COW project.%
...
From each of the query results of the 48 N1s described in Section~\ref{sec:queries}, we annotated one hundred cases with PL and approximately one hundred with NPL.
...
<<differenceplcoll, fig.pos="h!", fig.height=4.5, fig.width=4.5, fig.cap="Distribution of signed Cramérs $v$ scores for the internal and external plural data">>=
plot(x=NULL, y=NULL,
     xlim = c(0.5, 1.5),
     ylim=c(-0.5, 0.6),
     type="n", ann=FALSE, axes=F)
axis(1, at=c(0.85, 1.15), tick = F,
     labels=c("external plural", "internal plural"))
axis(2)
