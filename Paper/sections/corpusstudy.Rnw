...
Therefore, we had to manually annotate corpus exemplars containing N1+N2 compounds for whether they are plurals and whether a plural-enforcing semantic relation holds between N1 and N2.
...
First, a selection of N1s was required which represents the population of N1s well with regard to their productivity scores with PL and NPL, and a set of criteria was devised to make sure that that was the case.
We required that the potential productivity score with PL and with NPL not be $0$ or $1$, which excluded many very rare words.
...
The frequency class $c$ of a word $w$ increases with the word's token frequency $f(w)$, but calculation of the frequency class additionally relates this frequency to the frequency of the most frequent word in the corpus ($f^{max}$) and accounts for the power law distribution of word frequencies.%
...
Determining whether the semantics of N2 and the semantic relation holding between N1 and N2 forces N1 to have plural semantics was more intricate, and we found several classes of N2s triggering it.
...
Most prominently, there are N2 which denote a container of some sort.
...
In essence, this second large group contains compounds where the N2 might theoretically denote a container etc.\ for just one object, but both world knowledge and the context exclude this possibility beyond doubt.
...
The contingency table showing their bivariate distribution is given in Table~\ref{tab:ctextplural}.
...
However, despite the large sample, the p-value is relatively close to $0.05$, and the effect size is quite low at $v=\Sexpr{nice.float(cramer.v(extpl.chisq))}$.
Even worse, an inspection of the (standardised) residuals in Table~\ref{tab:ctextpluralresid} shows that the spurious effect even has the wrong direction, \ie PL is less frequent than expected under the null in plural compounds.
...
Figure~\ref{fig:corpuseffects} shows for each N1 the signed effect strength in the internal plural sub-study as its location on the horizontal dimension as well as the p-value through the colour coding.
...
However, even though the results for the last mentioned linking elements are negligible by themselves, it should still be noticed that \textit{all} N1s show a positive effect, except \textit{Ei} `egg' and \textit{Katze} `cat', which lean ever so slightly towards the negative side.
...
If there were no such preference, there should be roughly as many negative as positive $v$ scores, just like in the case of the external plural sub-study (see Figure~\ref{fig:differenceplcoll}).
