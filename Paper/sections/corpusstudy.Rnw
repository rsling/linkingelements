...
Therefore, we had to manually annotate corpus exemplars containing N+N compounds for whether they were plurals and whether a plural-enforcing semantic relation holds between N1 and N2.
...
First, a selection of N1s was needed which represents the population of N1s well with regard to their productivities with PL and NPL, and a set of criteria was devised to make sure that that was the case.
We required that the potential productivity score with PL and NPL be $0$ or $1$, which excluded many very rare words.
...
The queries to retrieve exemplars of compounds containing the chosen fifty N1s were made in DECOW16A using Python through the RStudio Server interface provided by the COW project.%
...
Due to some minor cleanups in the quality control process, the final sample size is not exactly $10,000$ but $\Sexpr{nice.int(sum(t.plot$n))}$.
...
%Furthermore, all annotations were made by the second author of this paper and a student assistant, but they were also by the first author, in order to minimise the amount of human annotation errors.}
