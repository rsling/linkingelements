...
<<corpusselection, fig.pos="htb!", fig.height=8, fig.cap="Selection of the N1s for the corpus study (triangles) in the context of all possible N1 candidates meeting the minimal criteria (dots); the x-axis and y-axis plot the potential productivity of N1 with PL and NPL, respecitvely; axes are on a logarithmic scale">>=
par(mfrow=c(3,3))
for (le in prod.plot.order) {
  ...
  plot(corpus.candidates[[le]]$Without_Ppot ~ corpus.candidates[[le]]$With_Ppot,
       ...
       main = .le.name,
       xlab = paste0("Pp with ", .le.name),
       ylab = paste0("Pp without ", .le.name)
  )
...
print(extpl.obs.xt, floating = T, booktabs = T, rotate.colnames = F)
@
...
print(extpl.resid.xt, floating = T, booktabs = T, rotate.colnames = F)
@
...
This means that in addition to quantifying the magintude of the effect size, they reflect whether for each N1 the co-occurrence of PL and plural semantics on N1 is preferred (positive sign) or dispreferred (negative sign).

The results confirm those of the overal test.
Only $\Sexpr{length(which(p.plot$p.sidak <= 0.05))}$ test reaches $\mathrm{sig}=0.05$.
The effect sizes are centered around $0$ with a mean of $\bar{v}=\Sexpr{nice.float(mean(p.plot$phi))}$ and a median of $\tilde{v}=\Sexpr{nice.float(median(p.plot$phi))}$ and a standard deviation of $s(v)=\Sexpr{nice.float(sd(p.plot$phi))}$.
...
It should be remembered from Section~\ref{sec:annotation} that we carefully distinguished between clear cases and leass clear cases, calling the two annotation schemes \textit{strict} and \textit{lax}.
...
<<differenceplcoll, fig.pos="htb!", fig.height=8, fig.cap="Distribution of signed Cramérs v scores for the internal and external plural conditions">>=
plot(x=NULL, y=NULL,
     xlim = c(0.5, 1.5),
     ylim=c(-0.5, 0.6),
...
print(intpl.obs.xt, floating = T, booktabs = T, rotate.colnames = F)
@
...
<<corpuseffects, fig.pos="htb!", fig.height=10, fig.cap="Individual per-N1 effect strengths and corrected p-values for the internal plural condition">>=
