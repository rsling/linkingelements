...
\hl{Was there a Dressler paper where he showed that PLs are not plural? Maybe I forgot.}
...
In sum, while the zero linkage is some kind of default, N1s from different declension classes have other preferences and are often blocked from occurring with certain linkages (partially depending on gender, stem-final segements, and the derivational status of N1).
...
While this does in no way exclude the possibility that a pluralic LE is a strong cue for plurality, the authors seem suggest that function and form have to stand in a one-to-one relationship except for a small number of lexicalised ``exceptions'' (\egg \citealt[42]{NeefBorgwaldt2012}).
