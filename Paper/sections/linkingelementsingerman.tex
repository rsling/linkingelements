...
Furthermore, \textcite[313--319]{Spencer1991} mainly discusses the \hl{izafet (this is a turkish word, right? does it need to be italicised? can you mention what it is/does or its internal construction?)} construction (with its marker \textit{-i}), which is arguably more phrasal than word formation processes, embedded in units that resemble compounds.
...
\hl{Also, certain LEs can replace N1-final segments.}
Last, N1s ending in schwa (graphemically \textit{<e>}) can drop this schwa in compounds.

In concrete analyses, we do not mark zero linkages in the simply concatenated compounds at all.
...
Several (soft) descriptive generalisations concerning linking element distribution can be made.
...
Despite the existence of more or less firm constraints on the selection of linkages, it is by no means fully pre-determined which linkage will be chosen in a given compound.
...
Our study focusses on N1s which alternate between a pluralic and a non-pluralic linkage, and it is our goal to show that \hl{they (does this refer to N1s or to linkages?)} actually have a plural interpretation and to clarify when this appears.
...
\footnote{We by no means wish to question the validity of the findings in \textcite{KoesterEa2004}, but there is also the possibility -- as with every experiment -- that the experiment accidentally failed to substantiate an effect (because ``a rare event occurred''; see Appendix~\ref{sec:onstatisticalanalysis}) or as an artefact of the experimental task. \hl{EP: I don't think we need this footnote, unless you think that we're going to offend Koester et al. by saying that in a different modality, things are different, or unless you want the link to the appendix.}}
...
Interestingly, \hl{no strong proponents of an interpretation of pluralic linkages as plural markers have so far weighed in (I don't think this is true. I mean, no one's said "Pl LE = pl suffix one to one", because that's just not true, but plenty of people, like the ones we've cited, suggest that there's a correlation, some more strongly than others. Can we rephrase this or delete it?)}, and no systematic empirical studies have (to our knowledge) been published.
...
From the \textit{Strict Layering Hypothesis} by \textcite{Siegel1979} through the \textit{Lexical Phonology} of \textcite{Mohanan1986} to the \textit{Words-and-Rules Theory} by \textcite{Pinker1999}, it was maintained that inflection applies after derivational word formation (including compounding), leading to inflectional affixes being positioned farther toward the edges of words than derivational affixes and to inflectional affixes not occurring between constituents of compounds.
...
We use the term ``systematic'' in the sense of Probabilistic Grammar, where preferences under specific conditions are not \hl{discrete but probabilistic, weighted, and better described as numerical rather than discrete [replace one of the "discrete"s - can you replace the second one with "binary" or something like that?]}.
